...
- 普通生产与自动化都可以在相邻生产方式之间双向试用；只有普通生产会在试运行失败后回滚，自动化和运输不会恢复旧生产方式。
...
- 普通生产的新投入品价格达到 +60% 时紧急回滚；自动化和运输观察结束后无条件保留并进入成功冷却。
...
低频普通生产继续使用上游的半年频率和错开月份；自动化、运输可按上游设置每月或每年两次扫描。候选与试运行始终每月推进；收益验收、紧急回滚和失败回滚仅适用于普通生产。
